...
export function App() {
  ...
  try {
    ...
    if (setDefaultActive && !getActiveTreeId() && list.length > 0) {
      setActiveTree(list[0].id ?? list[0].treeId ?? null);
    }

    const current = getActiveTreeId();
    if (current && !list.some((t) => (t.id ?? t.treeId) === current)) {
      setActiveTree(null);
    }
  } catch (e: any) {
  ...
  birth_year: (typeof birth_year === "number") ? Math.trunc(birth_year) : null,
  death_year: (typeof death_year === "number") ? Math.trunc(death_year) : null,
